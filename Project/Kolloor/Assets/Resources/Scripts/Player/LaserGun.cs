...
public class LaserGun
{
    ...
    public float dropforceBuilder = 1;
    ...
    public void Update()
    {
        ...
        {
            Vector3 vec = HoldingObject.transform.position;
            vec.x -= player.transform.position.x;
            vec.z -= player.transform.position.z;


            Debug.DrawLine(player.transform.position, player.transform.position + vec.normalized, Color.blue);
        }
    }
    ...
    private void DropCurrentObject(bool DropedBySlot = false)
    {
        ...
        if (!DropedBySlot)
        {
            ...
            Vector3 ShootPoint = player.transform.position;
            ShootPoint.y = 0;
            ShootPoint.Normalize();

            rigidBody.AddForce(ShootPoint * dropforceBuilder, ForceMode.Impulse);
        }
        dropforceBuilder = 1;
